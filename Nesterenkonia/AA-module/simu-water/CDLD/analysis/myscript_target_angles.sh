...
DIR="CG_angles"
mkdir $DIR
filendx=CDL_16_A15_181_A15_index_angles.ndx
NANGLES=$(grep "\[" $filendx | wc -l)

IANG=0
while [ $IANG -lt $NANGLES ]; do
    echo $IANG | gmx_mpi angle -f traj_AA2CG.xtc -n $filendx -ov $DIR/target_angle_$IANG.xvg -od $DIR/temp.xvg &> $DIR/temp_angles.txt
    echo "---- angle"$IANG" ----" >> $DIR/AA-data_angles.txt
  ##  awk '/< angle >/ {print $5} /Std. Dev./ {print $4}' $DIR/temp_angles.txt >> $DIR/AA-data_angles.txt
    gmx_mpi analyze -f $DIR/target_angle_$IANG.xvg -dist $DIR/target_histangle_$IANG.xvg -xvg none -bw 5.0 &> $DIR/temp2_angles.txt
