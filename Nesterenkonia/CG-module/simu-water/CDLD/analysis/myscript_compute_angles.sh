...
while [ $IANG -lt $NANGLES ]; do
    echo $IANG | gmx_mpi angle -f ../system_prod.xtc -n $filendx -ov $DIR/angle_$IANG.xvg -od $DIR/temp.xvg &> $DIR/temp_angles.txt
    echo "---- angle"$IANG" ----" >> $DIR/CG-data_angles.txt
   ## awk '/< angle >/ {print $5} /Std. Dev./ {print $4}' $DIR/temp_angles.txt >> $DIR/CG-data_angles.txt
    gmx_mpi analyze -f $DIR/angle_$IANG.xvg -dist $DIR/histangle_$IANG.xvg -xvg none -bw 5.0 &> $DIR/temp2_angles.txt
