...
!!=============================================================
            Program neighbors
              ...
              parameter (Nm=256)
              integer jnew(Nm,Nm),jmin,co(3,3),lip(3)
              ...
              double precision fco(3,3),favgl(3,3),favgt(3,3)
              character*36 dummy
              character*8 resnam(Nm),resid,res2,atom2,btype,bres
              character*7 atom,beadtype
!======================================================================!
             open(12,file='traj-com-upper.gro',action='read') ! !trajectory for center-of-mass of each lipid in upper leaflet   
             open(20,file='AMPG-timeline-neighborfractions-upper.dat')
             open(21,file='ALPG-timeline-neighborfractions-upper.dat')
             open(22,file='ACDL-timeline-neighborfractions-upper.dat')
             open(23,file='neighborfractions-upper.dat') !!timeaveraged
             favgt(:,:)=0.0
             do ite=1,maxit
               read(12,'(a)') dummy
               read(12,*) N
            ...
            do i=1,Nm
             ...
             do k=1,Nm
                  co(:,:)=0
                 if((resnam(k).eq.'AMPG')) ip=1
                 if((resnam(k).eq.'ALPG')) ip=2
                 if((resnam(k).eq.'ACDL')) ip=3
                 lip(ip)=lip(ip)+1
              do j=1,5
                 if((resnam(jnew(k,j)).eq.'AMPG')) jp=1
                 if((resnam(jnew(k,j)).eq.'ALPG')) jp=2
                 if((resnam(jnew(k,j)).eq.'ACDL')) jp=3
                 co(ip,jp)=co(ip,jp)+1
               enddo!j
              ...
              enddo!k
                  ...
                  write(20,*) favgl(1,1),favgl(1,2),favgl(1,3)
                  write(21,*) favgl(2,1),favgl(2,2),favgl(2,3)
                  write(22,*) favgl(3,1),favgl(3,2),favgl(3,3)
                  write(*,*) ite
         enddo !ite
         favgt(1,:) = favgt(1,:)/dble(maxit)
         favgt(2,:) = favgt(2,:)/dble(maxit)
         favgt(3,:) = favgt(3,:)/dble(maxit)
             write(23,*) favgt(1,1),favgt(1,2),favgt(1,3)
             write(23,*) favgt(2,1),favgt(2,2),favgt(2,3)
             write(23,*) favgt(3,1),favgt(3,2),favgt(3,3)
