...
mkdir $DIR
EXE=gmx
filendx=CDL_20_A15_20_A15_index_bonds.ndx
NBONDS=$(grep "\[" $filendx | wc -l)

IBOND=0
while [ $IBOND -lt $NBONDS ]; do
   echo $IBOND | $EXE distance -f traj_AA2CG.xtc -n $filendx -oall $DIR/target_bond_$IBOND.xvg  &> $DIR/temp_bonds.txt -len 0.6 -binw 0.010 -oh $DIR/target_histbond_$IBOND.xvg
    ...
    #$EXE analyze -f $DIR/target_bond_$IBOND.xvg -dist $DIR/target_histbond_$IBOND.xvg -xvg none -bw 0.002
